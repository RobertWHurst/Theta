...
export class Router {
  ...
  private _handle (patternStr: string, handler: Handler | Router, isErrorHandler: boolean): void {
    this._handlerChain
      ...
      : this._handlerChain = new HandlerChain(this._config, patternStr as string, handler, isErrorHandler)
  }

  private _unhandle (patternStr: string, handler: Handler | Router, isErrorHandler: boolean): boolean {
    if (!this._handlerChain) { return false }
    if (this._handlerChain.is(patternStr as string, handler, false)) {
      this._handlerChain = this._handlerChain.nextLink
      return true
    }
    return this._handlerChain.remove(patternStr as string, handler, isErrorHandler)
  }
}
