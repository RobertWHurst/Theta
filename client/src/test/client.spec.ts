...
describe("new ThetaClient(config?: Config)", () => {
  ...
  describe("#transport(transport: Transport): void", () => {
    ...
    describe("~handleMessage", () => {
      ...
      it("creates a context from the expanded and decoded message and routes it", async () => {
        ...
        await transport.handleMessage!(undefined)
  ...
  describe("#request(path: string, data?: any, handler: Handler): Promise<void>", () => {
    it("waits if not connected, and sends upon connection", async () => {
      ...
      sinon.assert.calledWith(
        send,
        sinon.match(/{"status":"","path":"[a-z0-9]+@path","data":"message1"}/),
      )
      sinon.assert.calledWith(
        send,
        sinon.match(/{"status":"","path":"[a-z0-9]+@path","data":"message2"}/),
      )
      sinon.assert.calledWith(
        send,
        sinon.match(/{"status":"","path":"[a-z0-9]+@path","data":"message3"}/),
      )
    })
    ...
    it("calls the handler once the client responds", async () => {
      ...
      await transport.handleMessage!(`{"status":"","path":"${channelId}@path","data":"response"}`)
    ...
    it.skip("times out if the client does not respond in time", async () => {
      ...
      const p = (client as any)._handleMessage(
        `{"status":"","path":"${channelId}@path","data":"response"}`,
      )
  ...
  describe("#handle(patternStr?: string, handlerOrRouter: Handler | Router): void", () => {
    it(
      "binds a handler to a given path, and only calls it if a matching " + "message is recieved",
      async () => {
        const client = new Client()
        const transport = transportFixture()
        client.transport(transport)
        ...
        await transport.handleMessage!(
          '{"status":"","path":"test/path/with/data","data":{"key":"value"}}',
        )
        sinon.assert.calledOnce(handler)
        sinon.assert.calledWith(
          handler,
          sinon.match
            .hasNested("params.variable", "data")
